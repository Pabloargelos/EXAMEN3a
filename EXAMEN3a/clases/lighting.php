<?
    ...
    class Lighting extends Conection{
        ...
        public function getAllLamps():array{
            $lamps = [];
            $sql = "SELECT lamps.lamp_id, lamps.lamp_name, lamp_on,lamp_models.model_part_number,lamp_models.model_wattage,zones.zone_name FROM lamps INNER JOIN lamp_models ONlamps.lamp_model=lamp_models.model_id INNER JOIN zones ON lamps.lamp_zone = zones.zone_id ORDER BY lamps.lamp_id;";
        ...
        public function drawLampList(){
            $show = $this->getAllLamps();
            for($i = 0; $i < count($show); $i++){
                echo 
                "<div class='element ". ($show[$i]->getEncendido())."'>
                <h4>
                <a href='changestatus.php?id=".$show[$i]->getId()."'><img src='img/bulb-icon-off.png'></a>
                </h4>
                <h4><a href='changestatus.php?id=''></a>".$show[$i]->getNombre()."</h4>
                <h1>".$show[$i]->getPotencia()."</h1>
                <h4>".$show[$i]->getZona()."</h4>
                </div>";
            }
        }

        public function getPotenciaZona():int{
            $sql = "SELECT SUM(lamp_models.model_wattage) as power FROM `lamps` INNER JOIN lamp_models ON lamp_model=lamp_models.model_id WHERE lamp_on = 1 ;";
        ...
        public function drawLampListZone($zona){
            if ($zona == 'all'){
                ...
                $show = $this->drawZoneOptions($zona);
                for($i = 0; $i < count($show); $i++){
                    echo 
                    "<div class='element ". ($show[$i]->getEncendido())."'>
                    <h4>
